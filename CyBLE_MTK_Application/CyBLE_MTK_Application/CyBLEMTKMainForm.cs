...
namespace CyBLE_MTK_Application
{
    public partial class CyBLE_MTK : Form
    {
        ...
        private void InitializeDUTInfo()
        {
            ...
            DataGridViewButtonColumn colItem3, colItem4;
            DataRow NewRow;
            DataView myView;
        ...
        private void MTKTestProgram_OnTestStopped()
        {
            ...
            {
                MessageBox.Show("shopfloor error" + ex.Message);
            }
        }
        ...
        private void MTKTestProgram_OnNextTest(int CurrentTest)
        {
            ...
            TestProgramGridView.ClearSelection();
            TestProgramGridView.Rows[CurrentTest].Selected = true;
            TestProgramGridView.Rows[CurrentTest].Cells["Status"].Value = "Running...";
        ...
        public void CyBLE_MTK_OnTestStatusUpdate(MTKTestMessageType MessageType, string Message)
        {
            ...
            {
                TestProgramGridView.Rows[MTKTestProgram.CurrentTestIndex].Cells["Status"].Style = new DataGridViewCellStyle { ForeColor = Color.Green, BackColor = Color.LightGreen };
            }
            else if (MessageType == MTKTestMessageType.Information)
            {
                TestProgramGridView.Rows[MTKTestProgram.CurrentTestIndex].Cells["Status"].Style = new DataGridViewCellStyle { ForeColor = Color.Black };
            }
            else if (MessageType == MTKTestMessageType.Complete)
            {
                TestProgramGridView.Rows[MTKTestProgram.CurrentTestIndex].Cells["Status"].Style = new DataGridViewCellStyle { ForeColor = Color.Black, BackColor = Color.LightGray };
            }

            TestProgramGridView.Rows[MTKTestProgram.CurrentTestIndex].Cells["Status"].Value = Message;
        }
        ...
        {
            if (CyBLE_MTK_Application.Properties.Settings.Default.PauseTestsOnFailure == true)
            {
            ...
            if (MTKTestProgram.TestProgram[MTKTestProgram.CurrentTestIndex].ToString() != "MTKTestProgramAll")
            {
        ...
        public bool CheckPemission()
        {
            ...
            for (int i = 0; i < DUTInfoDataGridView.Rows.Count; i++)
            {
                SerailNumber = DUTInfoDataGridView.Rows[i].Cells["Unique ID"].Value.ToString();
                Model = SerailNumber.Substring(0, 8);
                bool permission = m_SFCS.SFCS_PermissonCheck(SerailNumber, Model, "12345", "MTK");
                if (!permission)
                {
        ...
        public bool UploadResult()
        {
            ...
            for (int i = 0; i < DUTInfoDataGridView.Rows.Count; i++)
            {
                ...
                string TestResult = DUTInfoDataGridView.Rows[i].Cells["Status"].Value.ToString();
                bool upload = m_SFCS.SFCS_UploadTestResult(SerailNumber, Model, "12345", TestResult, "", TestResult, "MTK");
                if (!upload)
                {
                    MessageBox.Show(m_SFCS.connect_error, "Error:", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
